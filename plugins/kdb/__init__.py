...
from kdb.worker import Worker #REPLACE from calibre_plugins.kdb.worker import Worker
from metadata_compare import MetadataCompareKeyGen #REPLACE from calibre_plugins.kdb.metadata_compare import MetadataCompareKeyGen
from pre_filter_compare import PreFilterMetadataCompare #REPLACE from calibre_plugins.kdb.pre_filter_compare import PreFilterMetadataCompare
...
class Cbdb(Source):
    ...
    '''
    description = u'Download metadata and cover from kdb.cz'
    ...
    '''
    touched_fields = frozenset(['title', 'authors', 'tags', 'pubdate', 'comments', 'publisher', 'identifier:isbn', 'rating', 'identifier:kdb', 'languages'])
    ...
    def identify(self, log, result_queue, abort, title=None, authors=None, identifiers={}, timeout=30):
        ...
        try:
            ...
            for book_ref in entries:
                ...
                title_tag = ch[0].getchildren()[0].getchildren()[0]
                author_tag = ch[1].getchildren()
                if (len(author_tag[0].getchildren()) > 0):
                    author = author_tag[0].getchildren()[0].text
                else:
                    author = author_tag[0].text
                author = author.split(" ")[-1]
                tmp_ident = title_tag.get('href').split('-')[1]
                if tmp_ident == ident:
                    ident_found = True
    ...
    def create_query(self, title=None, authors=None, identifiers={}):
        ...
        if not q:
            return None
        return self.BASE_URL+'vyhledavani.html?'+urlencode({
            'q':q,
        })
